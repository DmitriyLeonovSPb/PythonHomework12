...
class Validate: #Используя дескрипторы проверяйте ФИО на первую заглавную букву и наличие только букв.
...
class Student:
    ...
    @lessons.setter
    def lessons(self, lessons_file: Path): #Названия предметов загружаются из файла CSV при создании экземпляра.
    ...
    def __call__(self, lesson: str, number: int, type_est: str = "lesson"): #Сохранение оценки по предмету
        ...
        if type_est == "lesson":
            ...
            #Средний балл по предмету/тесту
    ...
    @staticmethod
    def calculate_middle_estimate(lessons: dict) -> float: #Подсчёт средней оценки по предмету
        all_estimates = []
    ...
    @staticmethod
    def calculate_middle_test(lessons: dict) -> float: #Средний балл по всем предметам
        all_estimates = []
...
if __name__ == '__main__':
    student_1 = Student("Leonov", "Dmitriy", "Alexandrovich", Path('lessons.csv'))
    student_1("русский язык", 5)
    student_1("русский язык", 3)
    student_1("физика", 3)
    student_1("математика", 3)
    student_1("информатика", 99, "test")
    ...
    student_2 = Student("Petrov", "Petr", "Petrovich", Path('lessons.csv'))
